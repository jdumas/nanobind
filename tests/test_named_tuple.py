...
# ---------------------------------------------------------------------------
# Docstring API (class doc + per-field doc via .doc())
# ---------------------------------------------------------------------------
...
def test26_docpoint_default_still_applies():
    # The default-value setter still works when combined with .doc().
    p = t.DocPoint(5)
    assert p == (5, 0)
    p2 = t.DocPoint(1, 2)
    assert p2 == (1, 2)
...
def test28_function_signature_uses_python_name():
    ...
    # For a function taking a registered NamedTuple, the type name baked
    # into the caster's compile-time ``Name`` descr by ``NB_NAMED_TUPLE_CASTER``
    # should appear as a bare identifier -- not the ``%`` registry-lookup
    # placeholder, not a quoted forward ref, and not the demangled C++ name.
    sig = t.roundtrip_point.__nb_signature__
    sig_str = str(sig)
    assert "arg: Point," in sig_str and "-> Point" in sig_str
    assert '"Point"' not in sig_str  # quoted forward ref must not appear
    assert "%" not in sig_str  # unresolved registry placeholder
    assert "5Point" not in sig_str  # mangled Itanium ABI name prefix
    # Same check for the qualified-name macro path: ``geom::QualPoint`` is
    # bound with the explicit literal ``"QualPoint"``.
    qsig = str(t.roundtrip_qualpoint.__nb_signature__)
    assert "arg: QualPoint," in qsig and "-> QualPoint" in qsig
    assert "geom::QualPoint" not in qsig
...
# throwing default-value conversion). Both error paths surface at the
# register_named_tuple call site itself (eager construction; no separate
# finalize phase).
# ---------------------------------------------------------------------------
...
def test31_throwing_default_thunk_propagates():
    # A default-value conversion whose from_cpp fails must surface as a
    # Python exception at the register_named_tuple call site (rather than
